# Module 09: Parquet, Arrow, and Columnar Storage

## What you will learn

This module focuses on **Parquet, Arrow, and Columnar Storage** as part of Data Engineering. The goal is to understand the responsibility, use it in a versioned data platform that ingests, validates, transforms, serves, and monitors trustworthy datasets, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- Columnar formats store values by column so analytical engines can read only the fields a query needs.
- Parquet is a durable file format; Arrow is an in-memory columnar representation used for efficient interchange.
- Choose row groups, compression, and partition layout from measured access patterns.
...
```text
Topic: Parquet, Arrow, and Columnar Storage
...
Continue to [Module 10](../module-10-explicit-schemas-data-contracts-and-schema-evolution/README.md).