...
The project passes strict type checking and its behavior demonstrates this rule: v-model connects a form control value and its update event, but each control type still follows native HTML semantics and needs a label, validation, and an error path.