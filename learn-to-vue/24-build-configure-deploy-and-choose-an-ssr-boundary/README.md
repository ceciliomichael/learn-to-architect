...
## Install the runtime validator used below

```text
npm install zod
```

Zod runs in the built application, so it is a production dependency rather than a development-only tool. The `VITE_` prefix makes a value public to client code; it does not make a secret safe.
