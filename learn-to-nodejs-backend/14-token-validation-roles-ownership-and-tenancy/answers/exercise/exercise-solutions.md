...
Keep the automated test result and the smallest useful runtime artifact that proves Token Validation, Roles, Ownership, and Tenancy behaves correctly in a production-oriented TypeScript API service running on Node.js. The solution is complete only when another learner can reproduce both the successful behavior and the controlled failure without relying on private accounts or production data.