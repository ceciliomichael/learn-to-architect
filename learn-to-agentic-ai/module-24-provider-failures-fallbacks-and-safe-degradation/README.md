# Module 24: Provider Failures, Fallbacks, and Safe Degradation

## What you will learn

This module focuses on **Provider Failures, Fallbacks, and Safe Degradation** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- Classify provider failures as timeout, rate limit, invalid request, unavailable service, safety refusal, or malformed output before responding.
- A fallback model must pass the same contract and evaluation gates rather than silently changing behavior.
- Degrade to a limited honest feature when no safe provider result is available.
...
```text
Topic: Provider Failures, Fallbacks, and Safe Degradation
...
Continue to [Module 25](../module-25-document-ingestion-parsing-and-source-ownership/README.md).