# Module 15: Context Windows, Decoding, Hallucination, and Cost

## What you will learn

This module focuses on **Context Windows, Decoding, Hallucination, and Cost** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- Event time describes when the event occurred; processing time describes when the system handled it.
- Windows group unbounded events into finite results, while watermarks define how long late state is retained.
- A late-data policy must explain whether results update, events are dropped, or records enter a correction flow.
...
```text
Topic: Context Windows, Decoding, Hallucination, and Cost
...
Continue to [Module 16](../module-16-clients-servers-urls-dns-tls-and-http/README.md).