# Module 38: Deterministic Workflows and Model-Directed Decisions

## What you will learn

This module focuses on **Deterministic Workflows and Model-Directed Decisions** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- A workflow describes task dependencies and operational policy, while each task owns a bounded unit of work.
- A data interval describes the data a run should process and is different from the wall-clock start time.
- Keep large payloads in durable storage and pass only small references between tasks.
...
```text
Topic: Deterministic Workflows and Model-Directed Decisions
...
Continue to [Module 39](../module-39-manager-agents-handoffs-and-agents-as-tools/README.md).