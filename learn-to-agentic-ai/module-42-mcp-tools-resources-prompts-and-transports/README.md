# Module 42: MCP Tools, Resources, Prompts, and Transports

## What you will learn

This module focuses on **MCP Tools, Resources, Prompts, and Transports** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- A process has a lifecycle, an exit status, owned resources, and signals that request termination.
- Startup should fail clearly when required configuration is invalid.
- Readiness means the service can handle work; liveness only means the process should not be replaced.
...
```text
Topic: MCP Tools, Resources, Prompts, and Transports
...
Continue to [Module 43](../module-43-mcp-authorization-versioning-and-untrusted-servers/README.md).