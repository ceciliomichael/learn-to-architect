# Module 43: MCP Authorization, Versioning, and Untrusted Servers

## What you will learn

This module focuses on **MCP Authorization, Versioning, and Untrusted Servers** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- MCP uses host, client, and server roles with capability negotiation over a stateful JSON-RPC protocol.
- Servers can expose tools, resources, and prompts; the host remains responsible for model use and user policy.
- Remote capabilities require authentication, authorization, version checks, timeouts, and treatment as untrusted dependencies.
...
```text
Topic: MCP Authorization, Versioning, and Untrusted Servers
...
Continue to [Module 44](../module-44-prompt-injection-and-untrusted-retrieved-content/README.md).