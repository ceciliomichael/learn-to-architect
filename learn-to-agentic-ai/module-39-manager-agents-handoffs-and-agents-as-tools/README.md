# Module 39: Manager Agents, Handoffs, and Agents as Tools

## What you will learn

This module focuses on **Manager Agents, Handoffs, and Agents as Tools** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- Use deterministic code for known steps and model choice only where uncertainty provides value.
- A handoff changes the active specialist; an agent used as a tool returns a bounded result to its caller.
- Approval must pause before the side effect and preserve enough state to resume safely after a decision.
...
```text
Topic: Manager Agents, Handoffs, and Agents as Tools
...
Continue to [Module 40](../module-40-human-approval-pause-persistence-and-resume/README.md).