# Exercise Solution: Side Effects, Idempotency, and Approval Gates
...
Keep the automated test result and the smallest useful runtime artifact that proves Side Effects, Idempotency, and Approval Gates behaves correctly in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution. The solution is complete only when another learner can reproduce both the successful behavior and the controlled failure without relying on private accounts or production data.