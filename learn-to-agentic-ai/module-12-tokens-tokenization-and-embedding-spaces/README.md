# Module 12: Tokens, Tokenization, and Embedding Spaces

## What you will learn

This module focuses on **Tokens, Tokenization, and Embedding Spaces** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- Tokenization converts text into model units that affect length, cost, and boundaries.
- The context window is limited working input, not durable memory.
- Decoding selects among likely next tokens, so fluent output can still be unsupported or wrong.
...
```text
Topic: Tokens, Tokenization, and Embedding Spaces
...
Continue to [Module 13](../module-13-attention-and-transformer-structure/README.md).