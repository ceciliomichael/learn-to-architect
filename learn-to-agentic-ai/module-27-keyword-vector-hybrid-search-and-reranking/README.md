# Module 27: Keyword, Vector, Hybrid Search, and Reranking

## What you will learn

This module focuses on **Keyword, Vector, Hybrid Search, and Reranking** as part of Agentic AI Engineering. The goal is to understand the responsibility, use it in a controlled agent system with retrieval, tools, approval, evaluation, tracing, and durable execution, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- Retrieval first finds candidates, reranking orders them, and generation uses only the context selected for the request.
- Chunk boundaries, metadata, access filters, and update behavior determine whether relevant evidence can be found safely.
- A citation must identify the source that supports the nearby claim, not merely any retrieved document.
...
```text
Topic: Keyword, Vector, Hybrid Search, and Reranking
...
Continue to [Module 28](../module-28-access-aware-retrieval-citations-and-freshness/README.md).