# Exercise solution

```java
public class Main {
    public static void main(String[] args) {
        System.out.println("source");
        System.out.println("compile");
        System.out.println("run");
    }
}
```

Trace a program from readable source through bytecode to JVM execution. The program demonstrates each rule listed in the lesson and produces three lines: source, compile, run.

For the practice extension, change one behavior at a time and recompile immediately. If your implementation differs, it is still correct when it preserves validation, ownership, and observable behavior.

