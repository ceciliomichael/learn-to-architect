# Module 06: Sync and Async Work without Blocking the Event Loop

## What you will learn

This module focuses on **Sync and Async Work without Blocking the Event Loop** as part of Python Backend Development. The goal is to understand the responsibility, use it in a production-oriented Python API service using ASGI and FastAPI, and collect evidence that the result remains correct when input, dependencies, or timing change.
...
- ASGI defines the asynchronous interface between a Python server and application.
- Routers group transport concerns; dependencies should provide narrow services rather than hide global state.
- Use async for awaitable I/O, move blocking work away from the event loop, and keep CPU work bounded.
...
```text
Topic: Sync and Async Work without Blocking the Event Loop
...
Continue to [Module 07](../module-07-configuration-secrets-startup-and-shutdown/README.md).